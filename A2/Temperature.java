...
public class Temperature{
    ...
    //takes in temp input and converts it's to the proper value based on the scale. Checks if the temp is lower than abs zero.
   ...
   /** public char toChar(String scale){
        ...
        for (int i = 0; i<scales.length; i+=1){
            if (scale.length() > 1) {
                if (scale == scales[i]) {
                    Scale = charofScales[i];
                    break;
                }
            }
            else if (scale.length() == 1) {
                Character scaleAsChar = scale.charAt(0);
                if ( scaleAsChar == charofScales[i]) {
                    Scale = scaleAsChar;
                    break;
                }
            }
    ...
    public void setScale(String scale){
        ...
        if (newScale == 'K' && previousScale == 'C'){
            ...
            newtemp = (((previousTemp+459.67)*5)/9);
            selectedTemperature=newtemp;
        }
        else if (newScale == 'F' && previousScale == 'K'){
            newtemp = (((previousTemp*9)/5) - 459.67);
            selectedTemperature=newtemp;
        }
        else if (newScale == 'F' && previousScale == 'C'){
            newtemp = (((previousTemp*9)/5) + 32);
            selectedTemperature=newtemp;
        }
        else if (newScale == 'C' && previousScale == 'F'){
            newtemp = (((previousTemp-32)*5)/9);
            selectedTemperature=newtemp;
        }
        else {
     ...
     */
    // converts temp to new scale. pass scale to toChar to make sure the we compare chars and return chars
